...
**v1.3.0 热修补丁已完成并通过用户验收**（已沉淀至 RELEASE_NOTES，见 §6）。
当前**无进行中的开发任务**——等待用户逐条给出 v1.4.0 使用体验问题清单，据此规划实施。

> 背景：用户验收 v1.3.0 后，删除单个绑定文档时把整个后端工作目录删光（致命 bug）；曾导致整个
> 项目被毁，靠 GitHub 上的 v1.3.0 重新 clone 恢复，故本轮从 v1.3.0 起。该 bug 已于 2026-06-01 热修。

- v1.3.0 做了什么、改了哪些文件、6 个验收问题如何处置 → 已沉淀到
  **`RELEASE_NOTES.md`** + **`doc/项目当前情况.md`**（与代码一致的稳定快照）。
- 质量基线（热修后实测）：后端 `test_v120.py` **122/122**；`basedpyright` standard **0 error**；
...
## 6. v1.3.0 热修补丁（2026-06-01，已完成 + 已沉淀）

完整记录见 **`RELEASE_NOTES.md` 顶部「v1.3.0 热修补丁」段**。一句话回顾：

- **致命 bug**：`api/documents.py` 删除单个文档时 `shutil.rmtree(Path(upload_path).parent)`，绑定文档
  `upload_path` 为空串 → `Path("")→"."` → 删光后端 CWD。**已修**为按 `kb_id/doc_id` 显式拼路径、
  限定 `data/` 内、绝不删用户原始文件，并补齐 `rag_output/{doc_id}`、`mineru_zips/{doc_id}` 清理。
- **回归**：`test_v120.py::_test_delete_safety`——绑定文档删除不误删 CWD/原始文件；上传文档播种
  四表 + Qdrant 向量 + 派生目录后删除，断言全部清零。真机复查无孤儿。
- **文档清理**：移除 `doc/阿里云模型/` 子文件夹描述（改为 exa/context7 实时检索）。
- **改动文件**：`backend/app/api/documents.py`、`backend/test_v120.py`、`CLAUDE.md`、
  `RELEASE_NOTES.md`、`README.md`、`doc/项目当前情况.md`、`doc/文档导览.md`、本文件。
